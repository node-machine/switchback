...
var switchback = function(callback, defaultHandlers, callbackContext) {
  ...
  if (!_.isObject(callback) && !_.isFunction(callback)) {
    ...
    // Actually, instead let's make the new switchback an EventEmitter
    ...
    Switchback.on = function(evName, handler) {
      // When a listener is bound, add the handler so it is accessible to the caller
      Switchback[evName] = handler;
